...
import { z } from 'zod';
import { createSavedSearch, listSavedSearches } from '@spain/database';
...
export async function GET(request: Request) {
  ...
  return withAppAuthenticatedDb(session.userId, async (db) => {
    ...
    const items = await listSavedSearches(db, session.userId);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return await withAppAuthenticatedDb(session.userId, async (db) => {
      await ensureUserRow(db, session.userId);
      const item = await createSavedSearch(db, session.userId, body);
