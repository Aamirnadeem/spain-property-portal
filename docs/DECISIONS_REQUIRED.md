...
- **Decision:** Use exactly one of Inngest, Trigger.dev, or pg-boss for later workers. **Phase 1 uses `JOBS_PROVIDER=inline`** (no SaaS credentials). **Phase 3 planning default: pg-boss** (ADR-024); confirm in [`PHASE3_DECISIONS_REQUIRED.md`](PHASE3_DECISIONS_REQUIRED.md) D-P3-003 before implementation.
...
| D-002 | Exact job framework                                                                | Phase 1 uses **inline**. Phase 3 planning default **pg-boss** (ADR-024); confirm D-P3-003 before workers                   | Phase 3 workers                                 | Engineering lead             |
...
| D-009 | First live inventory partner                                                       | Named agency/developer/API; contract + media rights. **Note:** Idealista/Fotocasa URLs in legacy JSON do not satisfy this. | Phase 3 live import / MVP live-import gate      | Business development + legal |
